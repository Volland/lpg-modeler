...
Properties are flattened onto the subtype during resolution, each carrying the name of the type it came from, so an emitter never walks the chain and a diagram can say where a property is written. A parent's [[metamodel#Type Hierarchy#Mixins|mixins]] are flattened with its declared properties rather than after them, because a parent that is abstract reaches no artifact of its own and a subtype that took only what the parent declared would lose the rest outright. What is *not* inherited is stated where each concept is: [[metamodel#Open and Closed Types|openness]] and [[metamodel#Named Constraints|named constraints]] both stop at the type that declares them. An edge declared on an abstract parent belongs to every descendant, which is what makes the parent worth declaring; the emitters differ only in whether they can say so directly — see [[emitters#GQL Target]] against [[emitters#Ladybug Target]].
...
A mixin applied anywhere on the ancestor chain contributes to the type, exactly as a declared property does, and the flattened property names the mixin rather than the ancestor it was applied on — that is where a reader finds it written. Walking only an ancestor's declared properties was a real defect: an abstract parent reaches no artifact of its own, so its mixins' properties vanished outright on every target that flattens the hierarchy, while [[emitters#PG-Schema Target]] kept them through the declared type chain and disagreed. The type's own `mixins:` list stays its own, so the [[metamodel#Identity|key]], the `unused-mixin` warning and the targets that declare a mixin rather than flatten it all read the declaration rather than the closure. An unresolved mixin name is reported once, against the type that wrote it, rather than once per descendant.

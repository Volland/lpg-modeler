...
`lpg migrate` SHALL generate one migration script per requested target from the change set, name each `<stem>.<revision>.<target>.<extension>` where revision is the lockfile revision plus one written with four digits, and then rewrite the lockfile with the current model and that revision. When no target is given it SHALL generate for the ladybug, neo4j and falkordb targets. Requesting any other target SHALL be a `not-migratable` error. When the change set is empty the command SHALL write nothing and report that the model is unchanged.
...
- **THEN** `migrations/<stem>.0004.ladybug.cypher`, `<stem>.0004.neo4j.cypher` and `<stem>.0004.falkordb.sh` are written
